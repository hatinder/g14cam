...
{
    int k = n - 1;
    map<double, double> results;    //C++ Dictionary map with key
                                    // = x and value = calc results
    if (n == 0)
    {
        for (double i : v)
        {
    ...
    {
        map<double, double> PhiK = LegendrePolynomial(k, v);        //recursive
                                                            // call to get Phi(k)(x)
        map<double, double> PhiK_1 = LegendrePolynomial(k - 1, v);  //recursive
                                                            // call to get Phi(k-1)(x)
        for (double i : v)                          //iterate thru all values of x
        {
            ...
            results[i] = mFactor * (firstTerm - secondTerm); //results
                                            // to map with key=x and value=results
        }
    }
    return results;
}
...
vector<double> GaussQuadrature::NewtonMethod (int n)
{
    ...
    if (n % 2 == 1)      // Exploit the fact: for
        // n = odd number one root is zero. n=k+1
    {
        results[n - 1] = 0;
        m = n - 1;
    }
    ...
    for (int i = 0; i < m / 2; i++)
    {
        ...
        {
            //Numerator = Phi(x)
            auto Nr = LegendrePolynomial(n, initialGuess);
            //Denominator = dPhi(x) derivative
            auto Dr = LegendrePolynomialDerivative(n, initialGuess);
...
//        cout<<"Iteration: "<<j<<endl;
        ...
        //Exploit the fact: roots appear symmetrically about x=0
        results[k + 1] = -initialGuess[0];
        k = k + 2;
    }
    return results;
}
